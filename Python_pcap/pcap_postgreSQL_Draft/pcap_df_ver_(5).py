...
def f_l4 (packets_list):
    ...
    df_l4_options = pd.DataFrame({"TSVal_TSecr": [str(getattr(pkt['TCP'].options, field)) if pkt['TCP'].options is pkt else pd.NA
                            for pkt in packets_list]
                                                for layer, field_list in options.items()
                                                for field in field_list})
