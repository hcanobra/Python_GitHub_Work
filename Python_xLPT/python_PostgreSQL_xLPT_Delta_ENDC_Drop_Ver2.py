...
def f_import_csv ():
    v_file = '/Users/canobhu/Downloads/MTPL_5G_KPI_07182022105050.csv'
...
def f_delta(df,v_kpi):
    ...
    df_delta = df
    df_delta = df_delta[['DAY','MARKET','GNB','SITE','weekday',v_kpi]]

    df_delta['Diff_DELTA_%s'%v_kpi] = df_delta.groupby(['MARKET','GNB','SITE','weekday'])[v_kpi].diff().fillna(0)
    new_df = df_delta.groupby(['MARKET','GNB','SITE','weekday'])[v_kpi].agg({np.mean,np.median,np.std})
    df_stats = df_delta.merge(new_df, left_on=['MARKET','GNB','SITE','weekday'], right_index=True)
    
    df_stats = df_stats[df_stats['mean']>0]
    
    
    df_stats['out'] =  np.where(
                                (df_stats['5GNR_ENDC_Drop_pct'] <= (df_stats['median'] - (df_stats['std']*1)))
                                |
                                (df_stats['5GNR_ENDC_Drop_pct'] >= (df_stats['median'] + (df_stats['std']*1)))
                                ,True,False)
...
def f_day_week(df,v_kpi):
    df['DAY'] = pd.to_datetime(df['DAY'],format='%m/%d/%Y').dt.strftime('%Y/%m/%d')
...
#s = df.groupby('MARKET','GNB','SITE','weekday').DAY.diff().dt.days.ne(1).cumsum()
#df.groupby(['MARKET','GNB','SITE','weekday', s]).size().reset_index(level=1, drop=True)



df = df.loc[df['5GNR_ENDC_Drop_pct']>80]
df = df[['DAY','SITE','GNB','5GNR_ENDC_Drop_pct']]


df['Diff_DELTA_%s'%v_kpi] = df.groupby(['SITE','GNB','5GNR_ENDC_Drop_pct'])[pd.to_datetime(df['DAY']).dt.day_name()].count().fillna(0)

#new_df = df.groupby(['SITE','GNB','5GNR_ENDC_Drop_pct'])[pd.to_datetime(df['DAY'])].diff().fillna(0)


#df['Diff_DELTA_%s'%v_kpi] = df.groupby(['SITE','GNB','5GNR_ENDC_Drop_pct'])['DAY'].diff().dt.days.ne(1).cumsum()

#df.groupby(['MARKET','GNB','SITE','weekday', s]).size().reset_index(level=1, drop=True)


#new_df = df.groupby(['MARKET','GNB','SITE','weekday'])[v_kpi].agg({np.mean,np.median,np.std})
