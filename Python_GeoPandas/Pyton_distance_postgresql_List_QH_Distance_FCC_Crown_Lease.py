...
def f_postgresql_locations (v_sub_data):
    ...
    v_table = 'postgresql_HQ_DISTANCE_FCC_CROWN_LEASE'
...
def q_posgresql_target_query ():
    v_sql_commands = ('''
                    ...
                    SELECT * from 
                        public."MTNPLNS_2022_HQ_1519_LOCATIONS_geo"
                    WHERE "State" IN ('Utah','Montana','Idaho')
...
def q_posgresql_source_query ():
    v_sql_commands = ('''        
                    SELECT 
                        ...
                        lat,
                        lon
                    FROM public."FCC_ID_Towers_table_geo"
                    UNION
                    SELECT
                        'FCC_MT' as "Source",
                        "RecordId" :: TEXT AS id,
                        lat,
                        lon
                    FROM public."FCC_MT_Towers_table_geo"
                    UNION
                    SELECT
                        'FCC_UT' as "Source",
                        "RecordId" :: TEXT AS id,
                        lat,
                        lon
                    FROM public."FCC_UT_Towers_table_geo"
                    UNION
                    SELECT
                        'CROWN' as "Source",
